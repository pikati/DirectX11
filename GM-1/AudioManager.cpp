...
void AudioManager::Initialize()
{
	...
	if (FAILED(hr))
	{
		MessageBox(GetWindow(), "XAudio2オブジェクトの作成に失敗！", "警告！", MB_ICONWARNING);
	...
	if (FAILED(hr))
	{
		MessageBox(GetWindow(), "マスターボイスの生成に失敗！", "警告！", MB_ICONWARNING);
	...
	for (int nCntSound = 0; nCntSound < LABEL_MAX; nCntSound++)
	{
		...
		{
			MessageBox(GetWindow(), "サウンドデータファイルの生成に失敗！(1)", "警告！", MB_ICONWARNING);
			return ;
		}
		if (SetFilePointer(hFile, 0, NULL, FILE_BEGIN) == INVALID_SET_FILE_POINTER)
		{// ファイルポインタを先頭に移動
			MessageBox(GetWindow(), "サウンドデータファイルの生成に失敗！(2)", "警告！", MB_ICONWARNING);
			return ;
		}
		...
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "WAVEファイルのチェックに失敗！(1)", "警告！", MB_ICONWARNING);
			return ;
		}
		hr = ReadChunkData(hFile, &dwFiletype, sizeof(DWORD), dwChunkPosition);
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "WAVEファイルのチェックに失敗！(2)", "警告！", MB_ICONWARNING);
			return ;
		}
		if (dwFiletype != 'EVAW')
		{
			MessageBox(GetWindow(), "WAVEファイルのチェックに失敗！(3)", "警告！", MB_ICONWARNING);
			return ;
		}
		...
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "フォーマットチェックに失敗！(1)", "警告！", MB_ICONWARNING);
			return ;
		}
		hr = ReadChunkData(hFile, &wfx, dwChunkSize, dwChunkPosition);
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "フォーマットチェックに失敗！(2)", "警告！", MB_ICONWARNING);
			return ;
		}
		...
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "オーディオデータ読み込みに失敗！(1)", "警告！", MB_ICONWARNING);
			return ;
		}
		...
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "オーディオデータ読み込みに失敗！(2)", "警告！", MB_ICONWARNING);
			return ;
		}
		...
		if (FAILED(hr))
		{
			MessageBox(GetWindow(), "ソースボイスの生成に失敗！", "警告！", MB_ICONWARNING);
			return ;
		}
